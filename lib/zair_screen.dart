...
class ZairScreen extends StatelessWidget {
  const ZairScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: ArabicZairScreen(),
    );
  }
}

class ArabicZairScreen extends StatefulWidget {
  const ArabicZairScreen({super.key});

  @override
  _ArabicZairScreenState createState() => _ArabicZairScreenState();
}

class _ArabicZairScreenState extends State<ArabicZairScreen> {
  final FlutterTts flutterTts = FlutterTts();
  List<Map<String, String>> alphabets = [];
  List<Map<String, String>> alphabets2 = [];

  final String introTextEnglish = "Sound of";
  final String introTextArabic = "زير , اِ";
  ...
  Future<void> _loadAlphabetsFromXml() async {
    try {
      String xmlString = await rootBundle.loadString('assets/zair.xml');
  ...
  Future<void> _loadAlphabets2FromXml() async {
    try {
      String xmlString = await rootBundle.loadString('assets/zair2.xml');
