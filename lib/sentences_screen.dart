...
class ZabarScreen extends StatelessWidget {
  const ZabarScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      body: ArabicZabarScreen(),
    );
  }
}

class ArabicZabarScreen extends StatefulWidget {
  const ArabicZabarScreen({super.key});

  @override
  _ArabicZabarScreenState createState() => _ArabicZabarScreenState();
}

class _ArabicZabarScreenState extends State<ArabicZabarScreen> {
  final FlutterTts flutterTts = FlutterTts();
  List<Map<String, String>> alphabets = [];
  List<Map<String, String>> alphabets2 = [];

  final String introTextEnglish = "Sound of";
  final String introTextArabic = " زَبَرْ , اَ";
  ...
  Future<void> _loadAlphabetsFromXml() async {
    try {
      String xmlString = await rootBundle.loadString('assets/zabar.xml');
  ...
  Future<void> _loadAlphabets2FromXml() async {
    try {
      String xmlString = await rootBundle.loadString('assets/zabar2.xml');
